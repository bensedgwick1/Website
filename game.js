...
const passages = {
  ...
  "Cave": {
    ...
    text: "The air in the cave is damp and cold. As you step inside, a monstrous, snarling rat leaps from the shadows!",
    options: [], // Combat will generate options
    monster: "Giant Rat"
  },
  "CaveInterior": {
      title: "Cave Interior",
      image: "https://images.unsplash.com/photo-1519681393784-d120267933ba?auto=format&fit=crop&w=800&q=80",
      text: "With the rat defeated, you can now see ancient drawings on the walls, depicting a great city beyond the mountains.",
      options: [
  ...
  "CityGates": {
    ...
    "Catacombs_L1_Victory": {
        ...
        image: "https://images.unsplash.com/photo-1574933934529-3a8a13375753a8?auto=format&fit=crop&w=800&q=80",
        text: "The Skeletal Knight shatters into a pile of dust and cursed armor. Behind its throne, you see a gleaming sword and a passage leading deeper into the catacombs.",
        item: "Shadow Blade",
        options: [
            { text: "Take the Shadow Blade", target: "Catacombs_L1_Loot" },
...
const monsters = {
    "Giant Rat": {
        ...
        xp: 10,
        gold: 5,
        image: "https://images.unsplash.com/photo-1599669466277-681419a48919?auto=format&fit=crop&w=800&q=80",
    ...
    "Zombie": {
        ...
        xp: 20,
        gold: 12,
        image: "https://images.unsplash.com/photo-1580985290748-5246a4821a0a?auto=format&fit=crop&w=800&q=80",
...
function updateStats() {
  for (const stat in player.stats) {
    const val = clamp(player.stats[stat]);
  ...
  // Assuming you add a gold display element
...
function updateVisitList() {
  ...
  ul.innerHTML = '';
  Object.keys(player.visits).forEach(place => {
      if (!place.endsWith("_defeated")) {
        const li = document.createElement('li');
        li.textContent = `${passages[place] ? passages[place].title : place}`;
        ul.appendChild(li);
      }
  });
}
...
function renderPassage(passageName) {
  ...
  if (passage.effects) {
      for(const effect in passage.effects) {
          if(player.stats[effect] !== undefined) {
              player.stats[effect] = clamp(player.stats[effect] + passage.effects[effect]);
  ...
  if (monsterName && !player.visits[passageName + "_defeated"]) {
  ...
  storyDiv.innerHTML = `
    <article role="article" aria-label="${passage.title}">
      ...
      <nav class="nav-buttons" role="navigation" aria-label="Choices">
        ${availableOptions.map(opt => `<a href="#" data-target="${opt.target}" data-action="${opt.action}" data-shopid="${opt.shopId}">${opt.text}</a>`).join('')}
      </nav>
    </article>
  `;
...
function startCombat(monsterName) {
    ...
    const storyDiv = document.getElementById('story');
    let combatLog = `You are attacked by a ${monsterName}!`;
    ...
    function handleMonsterAttack() {
        ...
        player.stats.health = clamp(player.stats.health - monsterDamage);
    ...
    function handleFlee() {
        if (Math.random() > 0.5) {
            ...
            const previousPassage = Object.keys(player.visits).reverse()[1] || "Start";
            renderPassage(previousPassage);
        } else {
    ...
    function winCombat() {
        ...
        player.visits[player.currentPassage + "_defeated"] = true;
